...
const _compose = function _compose(fns, data) {
    return co(function* () {
        ...
        for(let i = 0; i < fns.length; i++) {
            ...
            const fn = fns[i];
            const passToFn = (i === 0) ? data : [result];
...
/**
 * Composes the given functions left to right.
 * If a function returns a promise the chain will wait for the promise to resolve
 * and continue with the resolved value. If you pass a generator function as a function
